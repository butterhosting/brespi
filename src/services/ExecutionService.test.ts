...
describe(ExecutionService.name, async () => {
  ...
  it("successfully executes a linear pipeline", async () => {
    ...
    const pipeline = await context.pipelineRepository.create(linearPipeline(steps));
    context.adapterServiceMock.submit.mockResolvedValue({
      artifacts: [],
      runtime: {},
    });
    // when
    ...
    steps.forEach((type, index) => {
      expect(context.adapterServiceMock.submit).toHaveBeenNthCalledWith(
        index + 1,
        expect.anything(),
  ...
  it("fails to execute a linear pipeline if there are errors", async () => {
    ...
    const steps = [Step.Type.postgresql_backup, Step.Type.compression, Step.Type.encryption, Step.Type.s3_upload];
    const pipeline = await context.pipelineRepository.create(linearPipeline(steps));
    context.adapterServiceMock.submit.mockImplementation((_artifacts, step, _trail) => {
    ...
    steps.forEach((type, index) => {
      if (index <= 2) {
        ...
        expect(context.adapterServiceMock.submit).not.toHaveBeenNthCalledWith(
          index + 1,
          expect.anything(),
          expect.anything(),
          expect.anything(),
  ...
  it("refuses to execute, if the pipeline is already executing", async () => {
    ...
    await context.executionRepository.create({
      id: "X",
      object: "execution",
      pipelineId: "123",
